"""
Open PGP signature verification using sequoia library
"""
...
class SignatureVerifierError(Exception):
    """Failed to initialize PGP signature verifier"""


class SignatureVerifier:
    """Verify PGP signatures using imported public key(s)"""

    def __init__(self, public_key_file: Path) -> None:
        self.logger = get_logger(__name__)
        self.public_key_file = public_key_file
        if not public_key_file.is_file():
            self.logger.error("PGP public key not found at %s", public_key_file)
            raise SignatureVerifierError("public key file not found")
    ...
    def _import_public_key(self, public_key_file: Path) -> None:
        ...
        try:
            ...
            if not self._certs:
                raise SignatureVerifierError("No valid certificates found in file")

        except Exception as err:  # pylint: disable=broad-exception-caught
            self.logger.error("Failed to import PGP public keys from %s: %s", public_key_file, err)
            raise SignatureVerifierError("failed to import public keys") from err
