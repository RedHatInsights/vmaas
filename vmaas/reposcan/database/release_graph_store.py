...
class ReleaseGraphStore:
    ...
    def store(self) -> None:
        ...
        try:
            cur.execute("SELECT name, graph, checksum FROM release_graphs")
            ...
            execute_values(
                cur,
                "INSERT INTO release_graphs (name, graph, checksum) VALUES %s",
                (to_insert,),
            )
            execute_values(
                cur,
                """
                    UPDATE release_graphs AS old
                    SET graph = new.graph, checksum = new.checksum
