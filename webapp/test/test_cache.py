...
def test_cache(monkeypatch):
    ...
    assert len(cache.arch2id) == 2
    assert len(cache.arch_compat) == 2
    assert len(cache.cve_detail) == 1
    assert len(cache.dbchange) == 5
    assert len(cache.errata_detail) == 1
    assert len(cache.errataid2name) == 1
    assert len(cache.errataid2repoids) == 1
    assert len(cache.evr2id) == 4
    assert len(cache.id2arch) == 2
    assert len(cache.id2evr) == 4
    assert len(cache.id2packagename) == 1
    assert len(cache.modulename2id) == 2
    assert len(cache.nevra2pkgid) == 5
    assert len(cache.package_details) == 5
    assert len(cache.packagename2id) == 1
    assert len(cache.pkgerrata2module) == 5
    assert len(cache.pkgid2errataids) == 5
    assert len(cache.pkgid2repoids) == 4
    assert len(cache.productid2repoids) == 1
    assert len(cache.repo_detail) == 1
    assert len(cache.repolabel2ids) == 1
    assert len(cache.src_pkg_id2pkg_ids) == 1
