...
class PackageStore: # pylint: disable=too-few-public-methods
    ...
    def _populate_packages(self, packages):
        ...
        if checksums:
            ...
            for pkg in packages:
                if (checksum_types[pkg["checksum_type"]], pkg["checksum"]) in checksums:
                    import_data.append((pkg["name"], evr_map[(pkg["epoch"], pkg["ver"], pkg["rel"])],
                                        archs[pkg["arch"]], checksum_types[pkg["checksum_type"]], pkg["checksum"], pkg["summary"], pkg["description"]))
            ...
            execute_values(cur,
                           """insert into package (name, evr_id, arch_id, checksum_type_id, checksum, summary, description) values %s
