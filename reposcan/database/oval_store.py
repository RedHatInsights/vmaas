...
class OvalStore(ObjectStore):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self):
        ...
        self.oval_object_version_map = self._prepare_table_map(cols=["oval_id"], table="oval_rpminfo_object",
                                                               to_col="version")
        self.oval_state_map = self._prepare_table_map(cols=["oval_id"], table="oval_rpminfo_state")
        self.oval_state_version_map = self._prepare_table_map(cols=["oval_id"], table="oval_rpminfo_state",
                                                              to_col="version")
    ...
    def _populate_data(self, entity, data, import_check_func, query, refresh_maps_func):
    ...
    def _populate_associations(self, entity_one, entity_many, ent_one_id_col, ent_many_id_col,
                               table_name, ent_one_id, ent_many_data, import_check_map,
                               missing_ok=False):
        ...
        try:
            ...
            for item in ent_many_data:
                ...
                if not item_id:
                    if not missing_ok:
                        self.logger.warning("Item (%s) not found: %s", entity_many, item["id"])
                    continue
    ...
    def _populate_states(self, oval_file_id, states):
        ...
        for state in states:
            if state["arch_operation"] is not None and state["arch_operation"] not in self.SUPPORTED_ARCH_OPERATIONS:
                ...
                continue
            if state["id"] in self.oval_state_map:  # Make sure state is imported
    ...
    def _test_import_check(self, item):
        ...
        if not check_existence_id:
            ...
            return None
        return item["id"], rpminfo_object_id, check_id, check_existence_id, item["version"]
    ...
    def _populate_tests(self, oval_file_id, tests):
        query = """insert into oval_rpminfo_test
                   ...
                   returning id, oval_id, version"""
        self._populate_data("tests", tests, self._test_import_check, query, self._test_refresh_maps)
        self._populate_associations("file", "tests", "file_id", "rpminfo_test_id", "oval_file_rpminfo_test",
                                    oval_file_id, tests, self.oval_test_map)
        for test in tests:
    ...
    def _definition_import_check(self, item):
        ...
        if item["version"] <= self.oval_definition_version_map.get(item["id"], -1):
            return None
        ...
        if item["criteria"]:
            try:
                cur = self.conn.cursor()
                criteria_id = self._populate_definition_criteria(cur, item["criteria"])
                self.conn.commit()
            except Exception: # pylint: disable=broad-except
    ...
    def _populate_definitions(self, oval_file_id, definitions):
        query = """insert into oval_definition
                   ...
                   version = EXCLUDED.version
                   returning id, oval_id, version"""
        self._populate_data("definitions", definitions, self._definition_import_check, query,
                            self._definition_refresh_maps)
        self._populate_associations("file", "definitions", "file_id", "definition_id", "oval_file_definition",
                                    oval_file_id, definitions, self.oval_definition_map)
        for definition in definitions:
            if definition["id"] in self.oval_definition_map:  # Make sure definition is imported
