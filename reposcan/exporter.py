...
class DataDump:
    ...
    def dump_package_details(self, dump):
        ...
        if self.packagename_ids:
            with self._named_cursor() as cursor:
                cursor.execute("""select id, name_id, evr_id, arch_id, summary, description, srpm_id
                                    from package
                                   where name_id in %s
                               """, [tuple(self.packagename_ids)])
                for pkg_id, name_id, evr_id, arch_id, summary, description, srpm_id in cursor:
                    dump["package_details:%s" % pkg_id] = (name_id, evr_id, arch_id, summary, description, srpm_id)
